...
void RegisterEcsMeshSystems(flecs::world& world)
{
	...
	world.observer<EntitySystem::ECS::RenderObjectPtr>().event(flecs::OnRemove)
		.each([&](EntitySystem::ECS::RenderObjectPtr& renderObject) {

		renderThread->ptr->EnqueueCommand(Render::ERC::DestroyRenderObject, (RenderCore::Geometry*) nullptr, renderObject.ptr);
	});
}
